...
class OptionPricing:
    ...
    def call_option_simulation(self):
        # 2 columns: the first column will store 0s and the second column will store the payoff
        ...
        # need S-E to calculate the max(S-E, 0)
    ...
    def put_option_simulation(self):
        # 2 columns: the first column will store 0s and the second column will store the payoff
        # need the first column of 0s: payoff function is max(0, E-S) for call option
        option_data = np.zeros([self.iterations, 2])
