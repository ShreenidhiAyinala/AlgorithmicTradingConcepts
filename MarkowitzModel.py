...
# generate random w weights (different portfolios)
...
def show_mean_variance(returns, weights):
    ...
    print("Expected portfolio volatility (std dev): ", portfolio_volatility)
...
# sum of w - 1= 0    f(x) = 0 is the function to minimize
...
def show_optimal_portfolio(opt, rets, portfolio_rets, portfolios_vols):
    plt.figure(figsize=(10,6))
    plt.scatter(portfolios_vols, portfolio_rets, c=portfolio_rets/portfolios_vols, marker='o')
    plt.grid(True)
    plt.xlabel('Expected Volatility')
    plt.ylabel('Expected Return')
    plt.colorbar(label='Sharpe Ratio')
